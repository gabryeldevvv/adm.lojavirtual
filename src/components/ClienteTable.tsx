...
export default function ClienteTable() {
  const { data: cliente = [], isLoading } = useClientes();
  ...
  return (
    <React.Fragment>
      ...
      <Sheet
        className="ClienteTableContainer"
        variant="outlined"
        sx={{
                  ...
                  onChange={(event) => {
                    setSelected(
                      event.target.checked
                        ? paginatedRows.map((row: ClienteRow) => row.id)
                        : []
                    );
                  }}
                  color={
          ...
          <tbody>
            {[...paginatedRows].sort(getComparator(ordem, "id")).map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  <Link level="body-xs" component={RouterLink} to={`/cliente/${row.id}`}  color="neutral" >
