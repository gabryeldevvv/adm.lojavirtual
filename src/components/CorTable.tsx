...
export default function CorTable() {
  const { data: cor = [], isLoading } = useCores();
                  ...
                  onChange={(event) => {
                    setSelected(
                      event.target.checked
                        ? paginatedRows.map((row: CorRow) => row.id)
                        : []
                    );
                  }}
                  color={
          ...
          <tbody>
            {[...paginatedRows].sort(getComparator(ordem, "id")).map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  <Link level="body-xs" component={RouterLink} to={`/cor/${row.id}`}  color="neutral" >
