...
export default function TamanhoTable() {
  const { data: tamanho = [], isLoading } = useTamanhos();
  ...
  return (
    <React.Fragment>
      ...
      <Sheet
        className="TamanhoTableContainer"
        variant="outlined"
        sx={{
                  ...
                  onChange={(event) => {
                    setSelected(
                      event.target.checked
                        ? paginatedRows.map((row: TamanhoRow) => row.id)
                        : []
                    );
                  }}
                  color={
          ...
          <tbody>
            {[...paginatedRows].sort(getComparator(ordem, "id")).map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  <Link level="body-xs" component={RouterLink} to={`/tamanho/${row.id}`}  color="neutral" >
